...
public class MissionOrganizer implements Iterable<Mission>
{
	...
	{
		if(mission.getCost() < ONE_TRILLION && !cheapMissions.contains(mission)) {
			cheapMissions.add(mission);
		}
		else if(!expensiveMissions.contains(mission)) {
	...
	{
		ArrayList<Mission> ms = new ArrayList<Mission>();
		ms.addAll(cheapMissions);
		ms.addAll(expensiveMissions);
		return ms.iterator();
	}
